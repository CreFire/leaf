...
func getPropertiesLocked(t reflect.Type) *StructProperties {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		p := new(Properties)
		name := f.Name
		p.init(f.Type, name, &f.Tag, &f, &prop.fixedSize)
		prop.Prop[i] = p
	}
...
func (p *Properties) init(typ reflect.Type, name string, tag *reflect.StructTag, f *reflect.StructField, fixedSize *int) {
	p.Name = name
	if f != nil {
		p.field = toField(f)
	}
	p.ver = -2
	if ver, ok := tag.Lookup(VersionFieldTagName); ok {
		v, err := strconv.Atoi(ver)
		if err == nil {
			p.ver = v
		} else {
			panic("cstruct: strconv.Atoi(" + ver + ") err: " + err.Error())
		}
	} else {
		if p.Name == VersionFieldName {
			p.ver = -1
		}
	}
...
func (p *Properties) setEncAndDec(typ reflect.Type, f *reflect.StructField, fixedSize *int) {
	...
	case reflect.Slice:
		*fixedSize += 2
		...
		case reflect.Struct: // [] struct
			if OptionSliceStructPointer {
				panic("cstruct: not pointer type. field name =" + f.Name)
			}
			p.stype = t2
			p.sprop = getPropertiesLocked(p.stype)
		...
		case reflect.Slice:
			...
			case reflect.Uint16:
				p.enc = (*Buffer).enc_slice_slice_uint16
				p.dec = (*Buffer).dec_slice_slice_uint16
				p.siz = (*Buffer).size_slice_slice_uint16
			case reflect.Uint32:
				p.enc = (*Buffer).enc_slice_slice_uint32
				p.dec = (*Buffer).dec_slice_slice_uint32
				p.siz = (*Buffer).size_slice_slice_uint32
			default:
				panic("cstruct: unknow type. field name = " + f.Name)
			}
		default:
	...
	case reflect.Array:
		...
		case reflect.Ptr: // [n]*struct
			switch t3 := t2.Elem(); t3.Kind() {
			case reflect.Struct:
				p.stype = t3
				p.sprop = getPropertiesLocked(p.stype)
				p.enc = (*Buffer).enc_array_substruct_ptr
				p.dec = (*Buffer).dec_array_substruct_ptr
				p.siz = (*Buffer).size_array_substruct_ptr
			default:
				panic("cstruct: unknow type. field name = " + f.Name)
			}
		default:
